...
import { RAGMemoryStore as _RAGMemoryStore } from '../DeepTreeEchoBot'
...
const mockGetMemoryForChat = jest.fn().mockReturnValue([])
const mockGetAllMemory = jest.fn().mockReturnValue([])
const mockGetConversationContext = jest.fn().mockReturnValue([])
const mockRetrieveRecentMemories = jest.fn().mockReturnValue([])
const mockSearchMemories = jest.fn().mockReturnValue([])
const mockClearChatMemories = jest.fn()
const mockSetEnabled = jest.fn()

jest.mock('../DeepTreeEchoBot', () => {
  const originalModule = jest.requireActual('../DeepTreeEchoBot')
  
  // Mock the RAGMemoryStore class
  class MockRAGMemoryStore {
    static getInstance() {
      return {
        storeMemory: mockStoreMemory,
        getMemoryForChat: mockGetMemoryForChat,
        getAllMemory: mockGetAllMemory,
        getConversationContext: mockGetConversationContext,
        retrieveRecentMemories: mockRetrieveRecentMemories,
        searchMemories: mockSearchMemories,
        clearChatMemories: mockClearChatMemories,
  ...
  return {
    ...originalModule,
    RAGMemoryStore: MockRAGMemoryStore,
  }
})
...
describe('DeepTreeEchoBot', () => {
  ...
  it('processes incoming messages correctly', async () => {
    ...
    ;(BackendRemote.rpc.getMessage as jest.Mock)
      .mockImplementation(() => Promise.resolve(mockMessage))(
        BackendRemote.rpc.getBasicChatInfo as jest.Mock
      )
      .mockImplementation(() => Promise.resolve(mockChatInfo))
    ...
    expect(mockStoreMemory).toHaveBeenCalledTimes(2) // Once for incoming, once for outgoing
  })
